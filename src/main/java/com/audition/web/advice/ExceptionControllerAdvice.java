...
public class ExceptionControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR) // Set the default status to 500 Internal Server Error

    ProblemDetail handleMainException(final Exception e, Logger logger) {
        logger.error("An unexpected error occurred while processing the request: {}", e.getMessage());
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR) // Set the default status to 500 Internal Server Error
    ProblemDetail handleSystemException(final SystemException e, Logger logger) {
        logger.error("An unexpected error occurred while processing the request: {}", e.getMessage());
